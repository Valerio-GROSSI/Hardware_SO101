...
Leave the `recalibrate` parameter set to `false` (default value), because recalibration waits for user input through `std::cin.get()`, which is not supported by this ROS 2 launch setup.
