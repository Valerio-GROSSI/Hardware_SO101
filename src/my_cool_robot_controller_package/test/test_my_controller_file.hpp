...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_CONTROLLER_FILE_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_CONTROLLER_FILE_HPP_
...
#include "my_cool_robot_controller_package/my_controller_file.hpp"
...
using ControllerStateMsg = my_cool_robot_controller_package::MyControllerFile::ControllerStateMsg;
using ControllerReferenceMsg = my_cool_robot_controller_package::MyControllerFile::ControllerReferenceMsg;
using ControllerModeSrvType = my_cool_robot_controller_package::MyControllerFile::ControllerModeSrvType;
...
class TestableMyControllerFile : public my_cool_robot_controller_package::MyControllerFile
{
  FRIEND_TEST(MyControllerFileTest, all_parameters_set_configure_success);
  FRIEND_TEST(MyControllerFileTest, activate_success);
  FRIEND_TEST(MyControllerFileTest, reactivate_success);
  FRIEND_TEST(MyControllerFileTest, test_setting_slow_mode_service);
  FRIEND_TEST(MyControllerFileTest, test_update_logic_fast);
  FRIEND_TEST(MyControllerFileTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = my_cool_robot_controller_package::MyControllerFile::on_configure(previous_state);
...
class MyControllerFileFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_my_controller_file/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_my_controller_file/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_my_controller_file")
  {
    ASSERT_EQ(
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_my_controller_file/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableMyControllerFile> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_CONTROLLER_FILE_HPP_
